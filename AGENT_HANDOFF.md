...
- **Next recommended**: `W70-T1 Governed Registry Operator Surface` is the next bounded registry wave. Use `docs/roadmaps/CVF_W70_T1_GOVERNED_REGISTRY_OPERATOR_SURFACE_EXECUTION_PLAN_2026-04-13.md` as the execution anchor and `docs/reference/CVF_W70_T1_GOVERNED_REGISTRY_OPERATOR_SURFACE_AGENT_PROTOCOL_2026-04-13.md` as the binding protocol. Correct focus: operator-facing consumption of existing filter/detail/retire capabilities, lifecycle readability, and retire/re-register guidance, while preserving route authority and bounded scope. This wave is prepared, not auto-opened; start only with fresh operator decision and explicit GC-018 authorization.
...
- **Next-agent boundary for W70-T1**: stay inside the governed registry operator-surface lane. Allowed focus: UI consumption of lifecycle/filter/detail/retire behavior, operator readability, retire action flow, and related docs/handoff alignment only. Do not modify `/api/execute`, provider adapters, PVV evidence, sandbox posture, storage backend class, or lifecycle semantics already delivered in W69 as part of this wave.
- **Binding protocol for the next agent is explicit**: before editing anything for `W70-T1`, read `docs/reference/CVF_W70_T1_GOVERNED_REGISTRY_OPERATOR_SURFACE_AGENT_PROTOCOL_2026-04-13.md`. If the next step does not make the governed registry easier to operate, easier to review, or easier to understand from the existing UI without widening backend scope, it does not belong in this tranche.
