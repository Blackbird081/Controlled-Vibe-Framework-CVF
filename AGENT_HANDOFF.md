...
- **W129-T1 — Noncoder Controlled Rollout And First Signal Capture CLOSED DELIVERED (2026-04-27)**: GC-018: `docs/baselines/CVF_GC018_W129_T1_NONCODER_CONTROLLED_ROLLOUT_AND_FIRST_SIGNAL_CAPTURE_AUTHORIZATION_2026-04-27.md` (depth 9/10). Roadmap: `docs/roadmaps/CVF_W129_T1_NONCODER_CONTROLLED_ROLLOUT_AND_FIRST_SIGNAL_CAPTURE_ROADMAP_2026-04-27.md`. CP0: Rollout playbook locked (`docs/reviews/CVF_W129_ROLLOUT_PLAYBOOK_2026-04-27.md`) — Stage A/B/C flag order, enable/hold/rollback criteria, minimum observation window. CP1: `NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR=true` enabled in `.env.local` (Stage A). CP2: `analytics.ts` AnalyticsEventType extended with `rollout_flag_enabled` + `rollout_session_start`; `home/page.tsx` fires both on mount with `sessionStorage` guard. CP3: Live release gate PASS 7/7; `entry_routing` lane exits `no_data` → `watch`. CP4: Signal capture doc `docs/reviews/CVF_W129_FIRST_SIGNAL_READOUT_2026-04-27.md` — 5 lanes remain `no_data` (documented reasons); `entry_routing` = watch; W130 NOT YET AUTHORIZED. **Verification (2026-04-27):** analytics 47/47 vitest pass; release gate PASS 7/7; GC-026: `docs/baselines/CVF_GC026_TRACKER_SYNC_W129_T1_CLOSED_2026-04-27.md`. Next: accumulate Stage A traffic → enable Stage B per playbook → re-read → open W130 when signal justifies.
