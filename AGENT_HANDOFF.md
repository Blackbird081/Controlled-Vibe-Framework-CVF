...
|---------|-------------|--------|
| W67-T1 CP1 | External Asset Productization — Contract and UX Hardening (REALIZATION class) | **ACTIVE 2026-04-13** — GC018-W67-T1-EXTERNAL-ASSET-PRODUCTIZATION authorized; `workflowStatus` (`invalid\|review_required\|registry_ready`) added to `ExternalAssetGovernanceResult` + route response; 5/5 targeted tests pass (all 3 status transitions covered); operator-facing UI page at `/governance/external-assets`; tsc clean; CP2 (registry handoff) and CP3 (review visibility + closure) pending |
...
- **Next recommended**: W67-T1 CP2 — Registry Handoff. CP1 (workflowStatus + operator UI) is ACTIVE/done. CP2 delivers a bounded storage/registry sink for approved `registry_ready_governed_asset` outputs. Key constraint: write path must be separated from `/api/execute` and PVV evidence files. Use `docs/roadmaps/CVF_W67_T1_EXTERNAL_ASSET_PRODUCTIZATION_EXECUTION_PLAN_2026-04-13.md` §CP2 as scope anchor. After CP2: CP3 (review visibility + workflow closure + canon handoff).
