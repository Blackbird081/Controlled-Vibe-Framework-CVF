...
- **Default next tranche: W91-T1** — Template Output Quality Benchmark. Corpus rescreen is now COMPLETE. Trusted benchmark subset is FROZEN at 9 wizards: `app_builder_wizard`, `business_strategy_wizard`, `content_strategy_wizard`, `data_analysis_wizard`, `marketing_campaign_wizard`, `product_design_wizard`, `research_project_wizard`, `security_assessment_wizard`, `system_design_wizard`. **`api_design` is REVIEW_REQUIRED — do NOT use as primary benchmark item.** Use Option A: replace with `build_my_app` (TRUSTED, development category) as the 10th benchmark item per D3. Alibaba-only baseline. 2-stage evaluation (machine precheck + human rubric). Requires fresh GC-018.
- **Next-agent boundary post-corpus-rescreen**: Corpus rescreen MET. Gate 1 MET. No active tranche. Fresh GC-018 required for W91-T1. Trusted benchmark subset is frozen — agents must not deviate from 9 wizards + `build_my_app` without a new rescreen decision.
