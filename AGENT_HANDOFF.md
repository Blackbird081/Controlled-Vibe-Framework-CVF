...
- **Next recommended**: if the operator wants the absorbed post-closure knowledge to become 100% part of CVF, open `W71-T1 Post-Closure Knowledge Native Adoption` as the next bounded wave. Use `docs/roadmaps/CVF_W71_T1_POST_CLOSURE_KNOWLEDGE_NATIVE_ADOPTION_ROADMAP_2026-04-13.md` as the execution anchor and `docs/reference/CVF_W71_T1_POST_CLOSURE_KNOWLEDGE_NATIVE_ADOPTION_AGENT_PROTOCOL_2026-04-13.md` as the binding implementation protocol. Correct focus: finish canon-native, governance-native, runtime-native, and quality-native adoption of the promoted `CVF ADDING NEW` + `Windows_Skill_Normalization` knowledge. This wave is prepared, not auto-opened; start only with fresh operator decision and explicit GC-018 authorization.
...
- **Next-agent boundary for W71-T1**: stay inside the post-closure knowledge native-adoption lane. Allowed focus: final posture classification for promoted docs, canon/whitepaper/readme alignment, bounded native adoption of the absorbed semantics across CPF/LPF/cvf-web surfaces, and the test/build proof needed to make that adoption explicit. Read the W71 roadmap and binding protocol before any edit. Do not reopen PVV/provider execution, `/api/execute`, storage migration, Docker/sandbox expansion, or unrelated architecture exploration as part of this wave.
