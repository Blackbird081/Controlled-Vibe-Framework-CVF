...
- **Post-review correction request (2026-04-14)**: N1-N4 are committed and targeted tests are green, but 2 canon-level corrections are still required before this lane should be called closure-clean again.
  1. **N2/N3 evidence-gate semantics must be reconciled.** The completion matrix defines `CVF-native core 100%` as requiring retrieval/default policy to be decided using trace-backed evidence rather than design preference (`docs/roadmaps/CVF_GRAPHIFY_LLM_POWERED_PALACE_CVF_NATIVE_COMPLETION_MATRIX_2026-04-14.md` §1, §5, §8). But W78 evidence remains explicitly `PROPOSAL_ONLY`, both promotion gates remain `NOT FULLY MET`, and the packet still claims `N2 closes the evidence gate` (`docs/baselines/CVF_W78_T1_BENCHMARK_EVIDENCE_PACKET_2026-04-14.md`). W79 then promotes this into whitepaper / tracker / handoff as `CVF-native core 100% gate CLOSED`. The next fixing agent must choose one consistent posture and apply it across matrix + W78 + W79 + whitepaper + tracker + handoff:
     - either **downgrade closure language** so N2/N3 are honest about evidence remaining insufficient for `core 100%`, or
     - **formally redefine the gate** so `HYBRID / NO SINGLE DEFAULT on PROPOSAL_ONLY basis` is explicitly a valid closure condition for this lane, then update the matrix and criteria so the new rule is canonically true rather than implied after the fact.
  2. **N1 retrieval-authority convergence is still ambiguous.** `rag.context.engine.convergence.contract.ts` now declares `declareKnowledgeNativeRetrievalAuthority()`, but the older `declareRagRetrievalAuthority()` remains unchanged and still declares the pre-knowledge-native path (`KnowledgeQuery -> KnowledgeRanking -> Retrieval -> ContextPackager`). Tests validate both declarations as if both are canonical. The next fixing agent must make the relationship explicit in source + tests + docs:
     - either mark the old W9 path as a legacy/raw-text subpath inside the knowledge-native authority, or
     - explicitly document that two authority declarations coexist and state which one is the canon top-level retrieval authority after W77.
  Until those 2 items are resolved, treat this lane as **committed / test-green / under canon-closure review**, not as an unquestioned final closure.
...
- **Next-agent boundary for this lane**: the default next uplift step is now a **CVF-native doctrine/governance documentation lane first** under `W72-T2`. Allowed scope:
...
- **Required sequence if this lane is reopened for real uplift**:
  ...
  2. Default to `W72-T2` (`doctrine/governance`) before considering any further implementation-capability follow-up.
  3. For `W72-T2`, start from `LLM-Powered` policy material first and treat `Graphify` as the already-landed implementation companion, not the doctrine lead.
  4. Only after `W72-T2` closes cleanly should a future `implementation-capability` or benchmark tranche be considered; if reopened later, `Graphify` structural-index enhancement remains the first implementation candidate, still as `Knowledge Layer` enhancement only.
...
- **Agent recommendation rule for this lane**: agents should not stall on "what should be next". The default recommendation is now `W72-T2 Knowledge Compilation Doctrine Uplift`. Human authorization still governs any fresh tranche opening, but the analytical queue-choice work is considered closed unless new evidence appears.
