...
Release closure rule: `python scripts/run_cvf_release_gate_bundle.py --json` must include the live governance E2E specs and must fail if `DASHSCOPE_API_KEY` is missing. `--e2e` is a targeted UI-only check, not a substitute for governance proof.
