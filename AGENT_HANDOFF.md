...
- **Recommended candidate ordering after W87-T1**: if the goal is immediate non-coder product value, prefer `(C) operator UI surface for guided response display` first; if the goal is broader runtime/provider evidence, prefer `(B) full 810-run multi-provider PVV batch`; keep `(A) extend guided responses` as a later bounded expansion unless fresh evidence shows a new dominant HIGH_RISK pattern gap.
