...
- **PRIORITY #1 ROADMAP (binding for all agents)**: `docs/roadmaps/CVF_NON_CODER_VALUE_REALIZATION_ROADMAP_2026-04-14.md` — every agent must read this before proposing any new direction. Execution order: **W90-T1** (HIGH_RISK pattern expansion to ≥8 patterns) → **W91-T1** (template output quality benchmark, 10 templates, false positive rate) → **W92-T1** (NEEDS_APPROVAL flow completion) → **W93-T1** (knowledge-native non-coder benefit validation) → **W94-T1** (risk visibility in UI) → W95/W96 (multi-step workflow, E2E benchmark). Multi-provider expansion is blocked until Gates 1–5 of the roadmap are all MET.
- **Default next tranche: W90-T1** — HIGH_RISK Pattern Expansion. Requires fresh GC-018. See roadmap for scope.
...
- **Next-agent boundary post-W89-T1**: no active tranche. The canon gap is closed. Any continuation requires fresh GC-018.
