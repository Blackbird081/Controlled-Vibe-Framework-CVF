...
- **Next recommended**: Requires fresh operator authorization + GC-018. Options: (A) `Knowledge Refactor step contract wave` — implement Step 6 (Refactor) of the 6-step lifecycle (triggered by accumulated maintenance signals; produces a new compiled artifact version); (B) `GC-026 benchmark execution` — promote PROPOSAL_ONLY knowledge benchmarks to trace-backed evidence; (C) any new knowledge or W7 layer capability. All are independent; none is default.
