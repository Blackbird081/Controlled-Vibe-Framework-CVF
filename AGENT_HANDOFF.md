...
- **Next recommended**: open `W68-T1 Governed Registry Hardening` as the next bounded wave. Use `docs/roadmaps/CVF_W68_T1_GOVERNED_REGISTRY_HARDENING_EXECUTION_PLAN_2026-04-13.md` as the execution anchor and `docs/reference/CVF_W68_T1_GOVERNED_REGISTRY_HARDENING_AGENT_PROTOCOL_2026-04-13.md` as the binding protocol. The next agent must follow the mandated order: duplicate/idempotency safety first, then read/detail behavior, then persistence posture/docs.
...
- **Next-agent boundary for W68-T1**: stay inside the governed registry hardening lane. Allowed focus: registry write safety, duplicate/idempotency behavior, registry read/detail usability, bounded file-persistence hardening, and related docs/handoff alignment only. Do not modify `/api/execute`, provider adapters, PVV evidence, sandbox posture, or external-asset semantics as part of this wave.
- **Binding protocol for the next agent is explicit**: before editing anything for `W68-T1`, read `docs/reference/CVF_W68_T1_GOVERNED_REGISTRY_HARDENING_AGENT_PROTOCOL_2026-04-13.md`. If the next step does not make the governed registry more reliable, more reviewable, or more legible, it does not belong in this tranche.
