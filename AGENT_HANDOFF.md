...
**W66-T1 CP3A Lane Bootstrap — GEMINI + ALIBABA DIRECT EVIDENCE CAPTURED; QVQ TEST PATH OPENED (2026-04-11). Lane manifest FROZEN. Gemini-2.5-flash pilot (5 calibration tasks, direct API) DONE — lane connectivity confirmed; CAL-004 ADVERSARIAL: CATASTROPHIC MISS on direct model (CFG-A behavior — approved deployment bypass without governance overlay); CAL-002/CAL-003/CAL-005 TRUNCATED. Important forensic note: current repo truth does NOT prove the Gemini truncation root cause. `max_tokens too low`, provider-side finish condition, safety interruption, and free-tier / quota behavior all remain OPEN hypotheses until rerun evidence captures provider diagnostics. Alibaba direct probe added two crucial findings: `qwen3.5-122b-a10b` ran cleanly on all 5 calibration tasks with `200 OK`, `finish_reason=stop`, and usage metadata present; `qvq-max-2025-03-25` returned `404 model_not_supported` on the current compatible-mode endpoint, so that is an endpoint-compatibility issue, not a quota/quality verdict. Web runtime now supports explicit Alibaba model override and QVQ-family streaming parsing, so another agent can test `qvq-max` immediately through cvf-web `/api/execute`; the dated snapshot `qvq-max-2025-03-25` still requires a different adapter/endpoint if that exact snapshot must run. Next: (1) run governed-path lanes through cvf-web `/api/execute`, (2) freeze explicit model parity for Alibaba governed-path testing, (3) open fresh GC-018 for full CP3A batch after governed-path evidence confirmed. Lane manifest: `docs/baselines/CVF_PRODUCT_VALUE_VALIDATION_LANE_MANIFEST_W66_T1_CP3A_2026-04-11.md`. Pilot evidence: `docs/baselines/CVF_PRODUCT_VALUE_VALIDATION_PILOT_EVIDENCE_W66_T1_CP3A_2026-04-11.md`.**
...
- **Alibaba QVQ test path** — `cvf-web /api/execute` now accepts explicit `model` override and Alibaba adapter support for QVQ-family streaming responses. Use `model: qvq-max` for the next runnable QVQ-lane test. Do not assume `qvq-max-2025-03-25` is runnable on the same endpoint; current canon still records `404 model_not_supported` for that snapshot.
...
- **Active lane bootstrap sequence (operator-confirmed 2026-04-11)**:
  1. Update run manifest (`docs/baselines/CVF_PRODUCT_VALUE_VALIDATION_RUN_MANIFEST_W66_T1_CP2_2026-04-11.md`) — keep `gemini` lane, set Alibaba direct-validated lane to `qwen3.5-122b-a10b`, and record `qvq-max-2025-03-25` as a separate blocked compatibility lane rather than a failed quality lane
  ...
  4. Run CAL-001 through CAL-005 calibration pilot tasks through the governed Alibaba lane only after freezing exact model parity (`qwen-turbo` default vs explicit model-override path). Do not silently substitute `qwen-turbo` for the already-validated direct `qwen3.5-122b-a10b` lane.
     Record provider diagnostics where available: HTTP status, finish reason, usage metadata, and quota / rate-limit signals
