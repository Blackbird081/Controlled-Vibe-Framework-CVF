...
- **W129-T1 — Noncoder Controlled Rollout And First Signal Capture AUTHORIZED (2026-04-27)**: GC-018: `docs/baselines/CVF_GC018_W129_T1_NONCODER_CONTROLLED_ROLLOUT_AND_FIRST_SIGNAL_CAPTURE_AUTHORIZATION_2026-04-27.md` (depth 9/10). Roadmap: `docs/roadmaps/CVF_W129_T1_NONCODER_CONTROLLED_ROLLOUT_AND_FIRST_SIGNAL_CAPTURE_ROADMAP_2026-04-27.md`. Scope: CP0 rollout playbook lock (flag order, enable/hold/rollback criteria); CP1 first flag enable (`NEXT_PUBLIC_CVF_INTENT_FIRST_FRONT_DOOR`) + readout baseline; CP2 `rollout_flag_enabled` + `rollout_session_start` analytics events; CP3 live-governed session batch → W128 lane readout exits `no_data` for entry_routing; CP4 signal capture doc (`CVF_W129_FIRST_SIGNAL_READOUT_2026-04-27.md`); CP5 handoff + GC-026 closure. Hard contracts: one flag per CP; W128 threshold contract binding; no governance path changes; W130 cannot open without measured signal.
