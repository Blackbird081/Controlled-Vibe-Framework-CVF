...
> State: **UNIFIED ON MAIN / W114 CLOSED / W115+W116+W117 PLANNED** — core architecture remains closure-assessed and closed-by-default; redesign wave `W105-T1` through `W109-T1` is delivered; strict front door is all-trusted (`42` skills / `50` linked templates / `50/50` trusted); W114 CP1-CP8 CLOSED DELIVERED: non-coder value is repeatable, visible, evidence-backed across web, workspace, and three project kinds; three new planned roadmaps: W115 Non-Coder Onboarding Experience, W116 Downstream Knowledge Pipeline, W117 D1.4b RAG Chunk Enforcement. Recommended execution order: W115 → W116 → W117. Any core reopen still requires a fresh `GC-018`.
...
#### Next Planned Tranches (W115 → W116 → W117)

Three new roadmaps are planned. Recommended execution order: **W115 → W116 → W117**.

- **W115-T1 — Non-Coder Onboarding Experience PLANNED**: roadmap `docs/roadmaps/CVF_W115_T1_NONCODER_ONBOARDING_EXPERIENCE_ROADMAP_2026-04-23.md`. Goal: a non-coder can reach first governed output in under 5 minutes without reading README. Scope: first-run state detection banner (CP1), "Try This" quick-path on top templates (CP2), progressive disclosure onboarding tour (CP3), friction audit with time-to-first-output evidence (CP4). Fully unblocked — no dependencies on W116 or W117. Changes stay inside `src/components/` and `src/app/(dashboard)/`; no governance runtime or API changes.

- **W116-T1 — Downstream Knowledge Pipeline PLANNED**: roadmap `docs/roadmaps/CVF_W116_T1_DOWNSTREAM_KNOWLEDGE_PIPELINE_ROADMAP_2026-04-23.md`. Goal: downstream workspace places `.md` files in `knowledge/` folder → governed runs inject that project knowledge → positive output delta on project-specific queries. Scope: manifest `knowledgePath` field (CP1), file ingestion + chunk indexing script (CP2), `POST /api/knowledge/ingest` API (CP3), execute-path integration (CP4), live evidence with positive delta (CP5). Extends `knowledge-context-injector.ts` and `knowledge-retrieval.ts`; does not add org/team scoping (that is W117). Prerequisite for W117 recommended but not required.

- **W117-T1 — D1.4b RAG Chunk Enforcement PLANNED**: roadmap `docs/roadmaps/CVF_W117_T1_D1_4B_RAG_CHUNK_ENFORCEMENT_ROADMAP_2026-04-23.md`. Goal: replace hardcoded `KNOWLEDGE_COLLECTIONS` constant with a writable JSON-file-backed store; admin can add/update/delete collections and chunks via API + UI at runtime. Scope: writable `KnowledgeStore` interface (CP1), admin CRUD API (CP2), admin UI add/edit/delete (CP3), execute-path uses writable store (CP4), Wave 2 live test regression `4/4 pass` (CP5). Retires the D1.4b deferred note. Depends on org/team scope enforcement already working (it does — Wave 1 delivered). Run after W116 to avoid building the persistent store before the ingestion path is proven.
