...
- **Reviewer correction — post-W84 canon sync still pending (2026-04-14)**: the W84-T1 packet itself is evidence-complete and governance-complete, but repo front-door canon is not yet fully synchronized. `docs/reference/CVF_MASTER_ARCHITECTURE_WHITEPAPER.md` still records `Last canonical closure = W83-T1` and `Current active tranche = NONE — W83-T1`, and `docs/reference/CVF_WHITEPAPER_PROGRESS_TRACKER.md` still records `latest governed closure W83-T1`, `Current active tranche = NONE — W83-T1`, and has no `W84-T1` tracker row. Until this narrow sync is completed, do **not** claim that master-architecture truth is fully aligned through W84 across all front-door canon surfaces.
- **Current knowledge-native lane status post-review**: lane W71–W84 is **benchmark-evidence complete but not yet canon-front-door closure-clean**. Absorption CLOSED, doctrine CLOSED, CPF capability CLOSED, N1+N2+N3+N4 CLOSED, live benchmark evidence PROMOTED to `LIVE_INFERENCE`, but whitepaper/tracker sync is still pending. There is a required correction tranche before the lane may be called globally closure-clean.
- **Binding instruction for future agents**: do not reopen benchmark design, rerun interpretation, N1/N2/N3 policy decisions, completion-matrix logic, or architecture reassessment for this lane. The only required follow-up is a narrow canon-sync correction wave that aligns whitepaper + tracker + handoff to the already-filed W84 truth.
- **Required next tranche for this lane**: `W85-T1 Post-W84 Canon Truth Sync` (DOCUMENTATION / CANON_SYNC class). Front-door roadmap: `docs/roadmaps/CVF_W85_T1_POST_W84_CANON_TRUTH_SYNC_ROADMAP_2026-04-14.md`. Scope is strictly limited to whitepaper/tracker/handoff sync. No live reruns. No policy changes. No new evidence claims. No reopening of W84 gates.
- **Next-agent boundary for this lane**: after W84-T1, the only default governed move is `W85-T1` until the canon sync gap is closed. After `W85-T1`, there should again be no default next step. Candidates beyond that remain: (A) PVV lane resume (810-run batch paused since W66-T1 CP3A); (B) any fresh new capability with a new GC-018. Neither is in scope for W85-T1.
