...
- **Default next tranche: W93-T1** — Knowledge-Native Non-Coder Benefit Validation. Use frozen 4-template set from measurement standard §8. Controlled comparison: same template/provider/model, only variable = `with knowledge-native` vs `without`. Requires fresh GC-018.
- **Next-agent boundary post-W92**: Gate 1 MET (W90-T1). Gate 2 MET (W91-T1). Gate 3 MET (W92-T1). No active tranche. Fresh GC-018 required for W93-T1. Trusted benchmark subset (9 wizards) remains frozen for W93-T1 subset use.
