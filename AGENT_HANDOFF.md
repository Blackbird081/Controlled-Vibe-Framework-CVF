...
- **Default next executable tranche**: `CVF_TEMPLATE_SKILL_CORPUS_RESCREEN_ROADMAP_2026-04-14.md` — corpus quality must be screened before benchmark truth. `GC-044` is already the active guard for template/skill quality; the next fresh GC-018 should open the corpus-rescreen wave unless the operator explicitly chooses a bounded W90 safety tranche first.
...
- **Default next tranche: W91-T1** — Template Output Quality Benchmark. Use 10 frozen templates from measurement standard §6.1; Alibaba-only baseline; 2-stage evaluation (machine precheck + human rubric). Prerequisite: corpus items must be classified TRUSTED_FOR_VALUE_PROOF per GC-044. Requires fresh GC-018.
- **Next-agent boundary post-W90-T1**: Gate 1 MET. No active tranche. Fresh GC-018 required for W91-T1.
