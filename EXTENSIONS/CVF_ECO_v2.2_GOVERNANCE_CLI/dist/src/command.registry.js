...
const execute_client_1 = require("./execute.client");
const governance_reliability_metrics_1 = require("./governance-reliability-metrics");
const operational_benchmark_suite_1 = require("./operational-benchmark-suite");
const product_outcome_runtime_1 = require("./product-outcome.runtime");
class CommandRegistry {
    ...
    registerBuiltins() {
        ...
        this.register({
            ...
            executeAsync: (args) => (0, execute_client_1.executeGovernedTemplateCommand)(args),
        });
        this.register({
            name: "run",
    ...
    helpCommand(args) {
        ...
        if (specificCmd) {
            const handler = this.handlers.get(specificCmd);
            if (handler) {
                return {
                    success: true,
    ...
    evaluateCommand(args) {
        const domain = args.flags.domain ?? args.flags.d ?? "general";
        const action = args.flags.action ?? args.flags.a ?? "read";
        const target = args.flags.target ?? args.flags.t ?? "unknown";
        const amount = parseFloat(args.flags.amount ?? "0");
    ...
    sessionCommand(args) {
        ...
        const agentId = args.flags.agent ?? "cli-agent";
        switch (subCommand) {
            case "start":
                return {
                    success: true,
    ...
    reportCommand(args) {
        ...
        const title = args.flags.title ?? "CVF Governance Report";
        return {
            success: true,
    ...
    auditCommand(args) {
        const sessionFilter = args.flags.session;
        const verdictFilter = args.flags.verdict;
        ...
        const filters = [];
        if (input)
            filters.push(`input=${input}`);
        if (sessionFilter)
            filters.push(`session=${sessionFilter}`);
        if (verdictFilter)
            filters.push(`verdict=${verdictFilter}`);
        return {
            success: true,
            message: countOnly
    ...
    runAliasArgs(args) {
        ...
        const runtimePlan = templateFromPositional
            ? (0, product_outcome_runtime_1.resolveProductOutcomeRuntimePlan)(templateFromPositional)
            : undefined;
        return {
            command: "execute",
            flags: {
    ...
    skillCommand(args) {
        const subCommand = args.positional[0] ?? "list";
        const registryPath = stringFlag(args, "registry") || product_outcome_runtime_1.CERTIFIED_SKILL_PACK_REGISTRY_PATH;
        if (subCommand === "plan") {
            const id = args.positional[1];
            if (!id)
                return { success: false, message: "Missing required skill pack id or outcome key.", exitCode: 1 };
            const plan = (0, product_outcome_runtime_1.resolveProductOutcomeRuntimePlan)(id, registryPath);
            if (!plan)
                return { success: false, message: `Product outcome runtime plan not found: ${id}`, exitCode: 1 };
            try {
                (0, product_outcome_runtime_1.assertProductOutcomeRuntimePlanFiles)(plan);
            }
            catch (error) {
        ...
        if (subCommand === "list" && args.flags.certified === true) {
            const plans = (0, product_outcome_runtime_1.listProductOutcomeRuntimePlans)(registryPath);
            return {
                success: true,
                message: args.flags.json === true
        ...
        if (subCommand === "show") {
            const id = args.positional[1];
            if (!id)
                return { success: false, message: "Missing required skill id.", exitCode: 1 };
            const skill = skills.records.find((record) => record.id === id);
            if (!skill)
                return { success: false, message: `Skill not found: ${id}`, exitCode: 1 };
            return {
                success: true,
    ...
    runtimePlanExecuteFlags(plan, rawInput) {
        return {
    ...
    receiptCommand(args) {
        ...
        const id = args.positional[1];
        if (!id)
            return { success: false, message: "Missing required receipt id.", exitCode: 1 };
        ...
        if (!match)
            return { success: false, message: `Receipt not found: ${id}`, exitCode: 1 };
    ...
    benchmarkCommand(args) {
        ...
        const format = formatFlag === "json" ? "json" : "table";
        const options = { input: input.trim(), format };
        try {
            const content = (0, node_fs_1.readFileSync)(options.input, "utf8");
            const events = subCommand === "operational"
                ? (0, operational_benchmark_suite_1.parseOperationalBenchmarkInput)(content)
                : (0, governance_reliability_metrics_1.parseAuditJsonl)(content);
            if (subCommand === "operational") {
                const report = (0, operational_benchmark_suite_1.buildOperationalBenchmarkReport)(events, options.input);
                return {
                    success: true,
                    message: (0, operational_benchmark_suite_1.formatOperationalBenchmarkReport)(report, options.format),
                    data: report,
                    exitCode: 0,
                };
            }
            const report = (0, governance_reliability_metrics_1.computeGovernanceReliabilityReport)(events);
            return {
                success: true,
    ...
    quickRiskScore(domain, action, amount) {
        const domainScores = {
...
function formatGovernanceReliabilityReport(report, format) {
    if (format === "json") {
...
function formatGovernanceBenchmarkRunReport(report, format) {
    if (format === "json") {
        return JSON.stringify(report, null, 2);
    }
    return [
...
function recordContainsValue(value, target) {
    if (value === target)
        return true;
    if (Array.isArray(value))
        return value.some((item) => recordContainsValue(item, target));
    if (value && typeof value === "object") {
        return Object.values(value).some((item) => recordContainsValue(item, target));
    }
    return false;
}
function formatMetricRate(rate, fractionDigits) {
...
function mergeRuntimePlanInput(rawInput, plan) {
...
function parseInputObject(rawInput) {
