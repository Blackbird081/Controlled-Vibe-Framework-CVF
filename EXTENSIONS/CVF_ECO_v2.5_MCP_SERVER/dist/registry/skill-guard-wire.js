 ...
 */
const RISK_NUM = { R0: 0, R1: 1, R2: 2, R3: 3 };
export class SkillGuardWire {
    mappings = new Map();
    registerSkill(mapping) {
    ...
    checkSkill(skillId, currentPhase, currentRisk, currentRole, activeGuardIds) {
        ...
        if (allowed) {
            ...
            const fixes = [];
            if (!phaseMatch)
                fixes.push(`advance to ${mapping.requiredPhase} phase`);
            if (!riskLevelSafe)
                fixes.push(`reduce risk level to ${mapping.minimumRiskLevel} or below`);
            if (!roleAuthorized)
                fixes.push(`switch to ${mapping.recommendedRole} role`);
            if (missingGuards.length > 0)
                fixes.push(`enable guards: ${missingGuards.join(', ')}`);
    ...
    getRequiredGuardsForSkill(skillId) {
        const mapping = this.mappings.get(skillId);
...
export function createDefaultSkillGuardWire() {
...
//# sourceMappingURL=skill-guard-wire.js.map