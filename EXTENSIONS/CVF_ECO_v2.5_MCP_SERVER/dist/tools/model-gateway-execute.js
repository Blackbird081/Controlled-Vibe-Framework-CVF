...
export const MODEL_GATEWAY_EXECUTE_ADAPTER_CONTRACT = 'cvf.mcpModelGatewayExecuteAdapter.rfrR3.v2';
...
function containsCredentialMaterial(value, seen = new WeakSet()) {
    ...
    return Object.entries(value).some(([key, nested]) => SECRET_KEY.test(key) || containsCredentialMaterial(nested, seen));
}
function rejected(code, message, traceId, admissionEvidence) {
    return {
...
function normalizeNativeRole(role) {
    return NATIVE_ROLE_MAP[role] ?? 'AI_AGENT';
}
function normalizeNativeRiskLevel(requestRiskClass) {
 ...
 */
function buildAdmissionContext(traceId, role, riskClass) {
...
function isMalformedAdmissionEvidence(traceId, evidence) {
    ...
    if (candidate.finalDecision !== 'ALLOW' &&
        candidate.finalDecision !== 'BLOCK' &&
        candidate.finalDecision !== 'ESCALATE') {
        return true;
    }
    if (candidate.finalDecision === 'BLOCK' &&
        (typeof candidate.blockedBy !== 'string' ||
            candidate.blockedBy.trim() === '')) {
        return true;
    }
    if (candidate.finalDecision === 'ESCALATE' &&
        (typeof candidate.escalatedBy !== 'string' ||
            candidate.escalatedBy.trim() === '')) {
        return true;
    }
    return false;
}
function isValidRiskClass(value) {
    return value === undefined || value === 'low' || value === 'medium' || value === 'high' || value === 'critical';
}
export async function executeModelGatewayAdapter(input, admission, executor) {
    let traceId;
    let prompt;
    let role;
    try {
        ...
        if (!traceId || !prompt || !role || !isValidRiskClass(input.requestRiskClass)) {
            return rejected('INVALID_GATEWAY_REQUEST', 'traceId, prompt, and agentRole must be non-empty strings; requestRiskClass must be valid when supplied.', traceId);
        }
    }
    catch {
        return rejected('INVALID_GATEWAY_REQUEST', 'MCP execution input could not be validated safely.');
    }
    if (!ALLOWED_ROLES.has(role)) {
        return rejected('ROLE_NOT_AUTHORIZED', 'Caller role is not authorized for this MCP tool.', traceId);
    }
    if (!admission) {
        return rejected('NATIVE_ADMISSION_NOT_CONFIGURED', 'Native CVF admission is not configured for this MCP server instance.', traceId);
    }
    let pipelineResult;
    try {
        ...
        return rejected('NATIVE_ADMISSION_EVALUATION_FAILED', 'Native CVF admission failed behind the credential boundary.', traceId);
    }
    try {
        if (isMalformedAdmissionEvidence(traceId, pipelineResult)) {
            return rejected('NATIVE_ADMISSION_EVIDENCE_INVALID', 'Native CVF admission returned malformed or trace-mismatched evidence.', traceId);
        }
    }
    catch {
        return rejected('NATIVE_ADMISSION_EVIDENCE_INVALID', 'Native CVF admission evidence could not be validated safely.', traceId);
    }
    ...
    if (admissionEvidence.decision === 'BLOCK') {
        return rejected('NATIVE_ADMISSION_BLOCKED', 'Native CVF admission blocked this request before Model Gateway execution.', traceId, admissionEvidence);
    }
    if (admissionEvidence.decision === 'ESCALATE') {
        return rejected('NATIVE_ADMISSION_ESCALATION_REQUIRED', 'Native CVF admission requires approval before Model Gateway execution.', traceId, admissionEvidence);
    }
    if (!executor) {
        return rejected('MODEL_GATEWAY_EXECUTOR_NOT_CONFIGURED', 'Model Gateway executor is not configured for this MCP server instance.', traceId, admissionEvidence);
    }
    const policy = {
        traceId,
        policyResult: 'allow',
    ...
    try {
        ...
        return {
            ...rejected('MODEL_GATEWAY_EXECUTION_FAILED', 'Model Gateway executor failed behind the credential boundary.', traceId, admissionEvidence),
            executorCalled: true,
        };
    }
}
export function registerModelGatewayExecuteTool(server, admission, executor) {
    server.tool(MODEL_GATEWAY_EXECUTE_TOOL, 'Execute through an injected CVF Model Gateway executor, gated by mandatory native CVF admission. Caller intent data cannot authorize execution; only a server-owned native admission ALLOW can. Fails closed when native admission or the executor is not configured.', {
        ...
        content: [{
                type: 'text',
                text: JSON.stringify(await executeModelGatewayAdapter(args, admission, executor), null, 2),
            }],
    })));
}
