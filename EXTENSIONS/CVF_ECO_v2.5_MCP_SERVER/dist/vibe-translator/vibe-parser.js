...
// ─── Main Parser ──────────────────────────────────────────────────────
export function parseVibe(input) {
...
function extractGoal(input, actionType) {
...
function detectMissingSlots(actionType, entities, constraints, input) {
    const missing = [];
...
function calculateConfidence(actionType, entities, constraints, missingSlots) {
    let score = 0.3; // Base
    if (actionType !== 'unknown')
        score += 0.25;
...
function suggestPhase(actionType) {
    switch (actionType) {
        case 'analyze':
        case 'search':
            return 'DISCOVERY';
...
function suggestRisk(actionType, constraints, entities) {
    if (actionType === 'deploy')
        return 'R3';
    if (actionType === 'delete')
        return 'R2';
    if (actionType === 'send' && entities.some((e) => e.type === 'money'))
        return 'R2';
    if (constraints.some((c) => c.type === 'security' && c.severity === 'hard'))
        return 'R2';
    if (actionType === 'send')
        return 'R1';
...
//# sourceMappingURL=vibe-parser.js.map