...
export function getPersonaQuestions(): PersonaQuestion[] {
  return [...PERSONA_QUESTIONS];
}

export function buildPersonaProfile(answers: Record<string, string>): PersonaProfile {
  const autonomy = (answers.autonomy || 'balanced') as AutonomyLevel;
  const riskMap: Record<string, CVFRiskLevel> = { R0: 'R0', R1: 'R1', R2: 'R2' };

  return {
    autonomyLevel: autonomy,
    riskTolerance: riskMap[answers.risk] || 'R1',
    preferredLanguage: (answers.language === 'vi' ? 'vi' : 'en') as 'en' | 'vi',
...
export function getMaxRiskForAutonomy(autonomy: AutonomyLevel): CVFRiskLevel {
  switch (autonomy) {
...
export function getDefaultRedLines(): RedLineConfig {
  return {
    dataAccessWhitelist: [],
...
export function mergeRedLines(base: RedLineConfig, custom: Partial<RedLineConfig>): RedLineConfig {
  return {
    dataAccessWhitelist: [
...
export function checkRedLine(action: string, config: RedLineConfig): {
  allowed: boolean;
...
export function completeOnboarding(
  ...
  customRedLines?: Partial<RedLineConfig>,
