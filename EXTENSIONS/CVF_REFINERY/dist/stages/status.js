 ...
 */
export function computeStatus(context, completedStageCount) {
    if (completedStageCount === 0) {
        return { status: "BLOCKED", failureTokens: ["REFINERY_NO_STAGES_EXECUTED"] };
    }
    const tokens = new Set();
    if (!context.schemaValid)
        tokens.add("SCHEMA_VALIDATION_FAILED");
    const blockingFindings = context.qualityFindings.filter((finding) => finding.severity === "BLOCK");
    const reviewFindings = context.qualityFindings.filter((finding) => finding.severity === "REVIEW");
    if (blockingFindings.length > 0)
        tokens.add("QUALITY_CHECK_FAILED");
    const integrityFailures = context.integrityResults.filter((item) => item.status === "FAIL");
    if (integrityFailures.length > 0)
        tokens.add("INTEGRITY_CHECK_FAILED");
    const lineageComplete = lineageIsComplete(context);
    if (tokens.size > 0) {
        return { status: "BLOCKED", failureTokens: [...tokens] };
    }
    const hasUnresolvedGovernance = reviewFindings.length > 0 ||
