 ...
 */
export function issueReference(receiptId, scope, version, validFromUtc, validUntilUtc, stores, deps) {
    ...
    if (receipt.evaluated_content_hash !== decision.packet_hash ||
    ...
    if (receipt.rule_version !== decision.rule_version ||
    ...
    const resolvedVerificationResults = receipt.verification_result_refs.map((id) => stores.verificationResults.get(id));
    if (resolvedVerificationResults.some((result) => !result)) {
        return { issued: false, reasons: ["VERIFICATION_RESULT_NOT_FOUND"] };
    }
    const blockingVerification = resolvedVerificationResults.some((result) => result.status === "FAIL" || result.status === "BLOCKED");
 ...
 */
export function computeCurrentReferenceState(referenceId, stores, nowUtcIso) {
 ...
 */
export function supersedeReference(oldReferenceId, newReferenceId, stores) {
    ...
    if (Number.isNaN(Date.parse(oldReference.valid_from_utc)) ||
