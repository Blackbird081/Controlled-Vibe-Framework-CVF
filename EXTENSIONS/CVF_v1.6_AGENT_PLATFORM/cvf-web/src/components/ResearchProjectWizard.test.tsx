...
vi.mock('./ProcessingScreen', () => ({
    ProcessingScreen: ({
        ...
        <div>
            ...
            <button onClick={() => onComplete('Live governed research output')}>CompleteProcessingMock</button>
...
describe('ResearchProjectWizard', () => {
    ...
    it('shows governed packet on review step', async () => {
        ...
        expect(await screen.findByText(/Governed research packet cho non-coder/i)).toBeTruthy();
    ...
    it('launches live governed path from review step', async () => {
        ...
        expect(await screen.findByText(/ProcessingScreenMock:Research Project Wizard/i)).toBeTruthy();
        expect(screen.getByText('Phase:BUILD')).toBeTruthy();
        expect(screen.getByText('Risk:R1')).toBeTruthy();
        expect(screen.getByText(/Skill:NONCODER_REFERENCE_PACKET:research-test-data/i)).toBeTruthy();
        ...
        expect(screen.getByText(/Live governed research output/i)).toBeTruthy();
        expect(screen.getAllByText(/Freeze receipt/i).length).toBeGreaterThan(0);
    });
});
