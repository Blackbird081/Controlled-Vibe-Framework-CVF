...
vi.mock('./ProcessingScreen', () => ({
    ProcessingScreen: ({
        ...
        <div>
            ...
            <button onClick={() => onComplete('Live governed system output')}>CompleteProcessingMock</button>
...
describe('SystemDesignWizard', () => {
    ...
    it('launches live governed path from review step', async () => {
        ...
        expect(await screen.findByText(/ProcessingScreenMock:System Design Wizard/i)).toBeTruthy();
        expect(screen.getByText('Phase:BUILD')).toBeTruthy();
        expect(screen.getByText('Risk:R2')).toBeTruthy();
        expect(screen.getByText(/Skill:NONCODER_REFERENCE_PACKET:system-test-data/i)).toBeTruthy();
        ...
        expect(screen.getByText(/Live governed system output/i)).toBeTruthy();
        expect(screen.getAllByText(/Freeze receipt/i).length).toBeGreaterThan(0);
    });
});
