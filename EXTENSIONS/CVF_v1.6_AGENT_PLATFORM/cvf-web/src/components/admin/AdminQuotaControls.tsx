  ...
  return (
    <section className="space-y-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Active Quota Rules</h3>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Append-only controls for team budgets. Soft cap warns, hard cap blocks, owner override expires after 24 hours.
          </p>
        </div>
        {feedback && (
          <div
              ...
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Soft {formatUsd(rule.softCapUSD)} • Hard {formatUsd(rule.hardCapUSD)} • {rule.period}
                  </div>
        ...
        <div className="space-y-5">
          <form
            ...
            onSubmit={(event) => {
              ...
              runAction(async () => {
                ...
                setFeedback({ tone: 'success', message: 'Quota policy saved.' });
                router.refresh();
              });
            }}
          >
            <div className="text-sm font-semibold text-gray-900 dark:text-white">Set Quota</div>
            <label className="block text-sm text-gray-700 dark:text-gray-300">
              <span className="mb-1 block">Team</span>
              <select
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="block text-sm text-gray-700 dark:text-gray-300">
                <span className="mb-1 block">Soft Cap (USD)</span>
                <input
                  type="number"
                  min="0.01"
                  step="0.01"
              ...
              <label className="block text-sm text-gray-700 dark:text-gray-300">
                <span className="mb-1 block">Hard Cap (USD)</span>
                <input
                  type="number"
                  min="0.01"
                  step="0.01"
            ...
            <label className="block text-sm text-gray-700 dark:text-gray-300">
              <span className="mb-1 block">Billing Period</span>
              <select
                className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950"
            ...
            >
              {isPending ? 'Saving...' : 'Save Quota Policy'}
            </button>
          </form>
            ...
            <form
              ...
              onSubmit={(event) => {
                ...
                runAction(async () => {
                  ...
                  setFeedback({ tone: 'success', message: 'Emergency override granted.' });
                  router.refresh();
                });
              }}
            >
              <div className="text-sm font-semibold text-gray-900 dark:text-white">Emergency Override</div>
              <label className="block text-sm text-gray-700 dark:text-gray-300">
                <span className="mb-1 block">Team</span>
                <select
                  className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950"
              ...
              <label className="block text-sm text-gray-700 dark:text-gray-300">
                ...
                <textarea
                  ...
                  disabled={isPending}
                  placeholder="Explain the operational reason for the temporary unblock."
                />
              </label>
              <button
                type="submit"
              ...
              >
                {isPending ? 'Submitting...' : 'Grant 24h Override'}
              </button>
            </form>
          )}
          ...
          {canGrantOverride && activeOverrides.length > 0 && (
            <div className="space-y-3 rounded-xl border border-gray-200 px-4 py-4 dark:border-gray-800">
              <div className="text-sm font-semibold text-gray-900 dark:text-white">Active Overrides</div>
                    ...
                    onClick={() => {
                      runAction(async () => {
                        await postJson('/api/admin/quota/override', 'DELETE', {
                          ...
                          reason: 'Owner revoked override from enterprise console.',
                        });
                        setFeedback({ tone: 'success', message: 'Override revoked.' });
                        router.refresh();
                      });
                    }}
