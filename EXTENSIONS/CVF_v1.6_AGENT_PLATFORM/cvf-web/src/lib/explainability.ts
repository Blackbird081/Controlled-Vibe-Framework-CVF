...
/**
 * CVF v1.7.3 — Explainability Layer (ported for Web UI)
...
export function explain(input: ExplainInput, locale: ExplainLocale = 'en'): HumanReadableExplanation {
    const actionLabel = INTENT_LABELS[locale][input.intentType];
    const riskMessage = RISK_LABELS[locale][input.riskLevel](input.riskScore);
...
/**
 * Map CVF risk levels (R0-R4) to explainability risk levels.
 */
export function mapCvfRiskLevel(cvfLevel: string): RiskLevel {
    switch (cvfLevel) {
        case 'R0': return 'LOW';
        case 'R1': return 'LOW';
        case 'R2': return 'MEDIUM';
        case 'R3': return 'HIGH';
        case 'R4': return 'CRITICAL';
        default: return 'MEDIUM';
    }
}

/**
 * Map decision codes to execution actions.
 */
export function mapDecisionToAction(decisionCode: string): ExecutionAction {
    if (decisionCode.includes('BLOCK') || decisionCode.includes('REFUSAL_BLOCK')) return 'BLOCK';
    if (decisionCode.includes('APPROVAL') || decisionCode.includes('ESCALATE')) return 'ESCALATE';
    return 'EXECUTE';
}
