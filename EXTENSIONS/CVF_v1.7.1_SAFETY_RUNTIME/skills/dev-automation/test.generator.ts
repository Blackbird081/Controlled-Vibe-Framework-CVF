...
export async function generateTestsForArtifact(
  ...
  const mergedFileChanges = [
    ...artifact.fileChanges,
    ...testFileChanges,
  ];

  const updatedChecksum = computeChecksum(
    artifact.proposalId,
    mergedFileChanges
  );
  ...
  return {
    ...
    metrics: {
      tokensUsed:
        artifact.metrics.tokensUsed + result.tokensUsed,
      generationTimeMs:
        artifact.metrics.generationTimeMs +
        (endTime - startTime),
      modelUsed:
        artifact.metrics.modelUsed +
        " + " +
        result.modelUsed,
