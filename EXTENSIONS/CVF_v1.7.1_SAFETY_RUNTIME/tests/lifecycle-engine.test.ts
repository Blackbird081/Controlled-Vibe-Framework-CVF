...
describe("LifecycleEngine Integration", () => {
  ...
  function registerTestPolicy(version: string, rules: PolicyRule[]) {
    ...
    registerTestPolicy(v, [
      {
        ...
        evaluate: (p) => ((p.estimatedCost as number) <= 100 ? "approved" : null),
      },
    ])
    ...
    registerTestPolicy(v, [
      {
        ...
        evaluate: (p) => ((p.estimatedCost as number) > 10000 ? "rejected" : null),
      },
    ])
    ...
    registerTestPolicy(v, [
      {
        ...
        evaluate: (p) => (p.action === "deploy" ? "approved" : null),
      },
    ])
  ...
  it("should emit proposal:submitted event", async () => {
    ...
    expect((submitted?.data as { proposalId: string }).proposalId).toBe(`t-4-${v}`)
  })
