...
import { validate, ProposalEnvelopeSchema, LoginRequestSchema, AISettingsSchema, OpenClawMessageSchema, LifecycleInputSchema } from "../validation/schemas"
...
describe("ProposalEnvelopeSchema", () => {
    ...
    it("should reject invalid UUID", () => {
        const result = validate(ProposalEnvelopeSchema, { ...validProposal, id: "not-a-uuid" })
        expect(result.success).toBe(false)
    })

    it("should reject invalid source", () => {
        const result = validate(ProposalEnvelopeSchema, { ...validProposal, source: "unknown" })
        expect(result.success).toBe(false)
    })
...
describe("LoginRequestSchema", () => {
    it("should accept valid login", () => {
        const result = validate(LoginRequestSchema, { username: "admin", password: "secureP@ss1" })
        expect(result.success).toBe(true)
    })
    ...
    it("should reject short username", () => {
        const result = validate(LoginRequestSchema, { username: "ab", password: "secureP@ss1" })
        expect(result.success).toBe(false)
    })
})
