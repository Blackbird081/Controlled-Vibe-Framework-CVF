...
LEGACY_ASSET_DIR = os.path.join(BASE_DIR, "legacy", "streamlit_assets")


def resolve_asset(file_name):
    candidates = [
        os.path.join(BASE_DIR, file_name),
        os.path.join(LEGACY_ASSET_DIR, file_name),
    ]
...
def apply_vibe():
    ...
    bg_path = resolve_asset("background.jpg")
    if bg_path:
...
if st.session_state.audio_to_play:
    file_path = resolve_asset(st.session_state.audio_to_play)
    if file_path:
