...
| MinerU document-intelligence foundation | Private provenance has bounded foundation-chain work for receipt, quality, and memory-route candidate surfaces. | Public summary only; no private output, runtime extraction route, production memory/RAG release, provider/live proof, or use-case/legal workflow claim. |
...
- a claim that MPI-T6 runtime memory access, vector/durable storage,
  ...
  by default;
- a claim that use-case adapter or workspace-package material is activated,
  certified, runtime-bridged, or exported as raw private package material;
