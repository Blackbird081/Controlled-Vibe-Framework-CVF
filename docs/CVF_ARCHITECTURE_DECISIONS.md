...
**Decision** Adopt repository exposure classification as a second mandatory precondition for pre-public planning. Visible repository roots and extension roots must declare an exposure posture such as `PUBLIC_DOCS_ONLY`, `PUBLIC_EXPORT_CANDIDATE`, `INTERNAL_ONLY`, or `PRIVATE_ENTERPRISE_ONLY`, and `GC-038` enforces that classification before any public mirror, export package, reduced-core, or full-public decision proceeds.
...
**Consequences** CVF now defaults to private core posture unless a later distribution decision explicitly authorizes something narrower or broader. Future packaging waves must choose a publication model intentionally rather than inheriting one from repository layout.
