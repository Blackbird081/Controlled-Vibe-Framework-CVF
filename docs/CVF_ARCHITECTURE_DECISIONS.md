...
Date: `2026-04-02` | Status: `Active` | Branch: `cvf-next` | Layer: `Governance Platform` | Related commits: *(local, current pre-public exposure batch)*
**Context** Lifecycle classification makes repository cleanup safer, but it does not answer the separate question of what may ever become public. CVF also needs to protect enterprise/private IP and avoid conflating folder cleanup with access control. A public GitHub repository can be cloned as a whole, so selective publication cannot be achieved by layout alone.
**Decision** Adopt repository exposure classification as a second mandatory precondition for pre-public planning, then add `GC-039` as the stop-boundary before any `P3` relocation authorization. Visible repository roots and extension roots must declare exposure posture through `GC-038`, while root files, `PUBLIC_DOCS_ONLY` root audit status, `PUBLIC_EXPORT_CANDIDATE` export-readiness, phase-gates, and memo freshness must be explicit before relocation is even discussable.
**Rationale** Architecture readability and publication exposure are related but distinct concerns. Without an explicit exposure model, later cleanup work could accidentally over-expose internal governance assets, product surfaces, or enterprise-only modules.
**Consequences** CVF now defaults to private core posture unless a later distribution decision explicitly authorizes something narrower or broader. Future packaging waves must choose a publication model intentionally rather than inheriting one from repository layout, and `P3` remains blocked until readiness truth is explicit enough to support structural movement safely.
