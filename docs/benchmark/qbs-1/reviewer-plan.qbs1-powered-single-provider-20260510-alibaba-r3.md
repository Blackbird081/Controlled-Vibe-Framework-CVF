# QBS-1 Reviewer Plan - Alibaba Powered Single Provider R3
...
This reviewer plan is frozen for the R3 aggregate-only
...
`EXECUTION_FAILED_NO_QBS_SCORE`. The unexecuted R2 pre-registration remains a
no-score planning artifact.
...
Output-quality reviewers must not see config labels, provider/model
identifiers, CVF receipt fields, governance metadata, cost or latency metadata,
or F7 front-door route internals except where reviewing clarification behavior.
