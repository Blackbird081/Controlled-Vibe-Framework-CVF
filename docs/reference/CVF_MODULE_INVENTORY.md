...
| `EXTENSIONS/CVF_EXECUTION_PLANE_FOUNDATION/` | execution-plane coordination shell | whitepaper completion `W2-T1 / CP1-CP2` | local-ready | first execution-plane shell preserving lineage for MCP bridge, gateway, adapter, memory, prompt-preview, and explainability surfaces, now with explicit shell-facing gateway/MCP wrapper boundaries while deferring MCP runtime internals |
