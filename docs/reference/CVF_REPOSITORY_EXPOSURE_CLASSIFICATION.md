...
Freeze-in-place retained/internal roots:

- `CVF_SKILL_LIBRARY` and `ui_governance_engine` remain at the visible repo root on `cvf-next`
- their exposure posture is still `PRIVATE_ENTERPRISE_ONLY`
- the isolated `P3/CP2` relocation packet remains historical evidence only and is excluded from canonical landing under `GC-039`
