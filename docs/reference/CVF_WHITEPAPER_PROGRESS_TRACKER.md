...
> Last refreshed: `2026-04-17` (latest governed closure `W100-T1 CLOSED DELIVERED`; NC_001 regex expanded; OFU-2 closed; E2E metrics 5/5 PROVEN; no active tranche)
...
| Current active tranche | `NONE — W100-T1 CLOSED DELIVERED 2026-04-17; OFU-2 FIXED; E2E Value PROVEN 5/5 metrics met; next tranche requires fresh GC-018 authorization` |
...
| `W71-T1` Post-Closure Knowledge Native Adoption (DOCUMENTATION class) | `CLOSED DELIVERED 2026-04-13 — absorbed knowledge from Graphify / LLM-Powered / Palace sources; 6 canonical + 1 bounded invariant + 1 reference appendix artifacts promoted; W7/CLI family deferred by design; CVF-native external-asset governance lane confirmed official surface` |
| `W72-T1` StructuralIndexContract (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — StructuralIndexContract + StructuralIndexBatchContract in CPF Knowledge Layer; BFS 2-hop traversal; governed inputs required; indexHash content-bound; knowledge barrel updated` |
| `W72-T2` Knowledge Compilation Doctrine Uplift (DOCUMENTATION class) | `CLOSED DELIVERED 2026-04-14 — CVF_COMPILED_CONTEXT_GOVERNANCE_POLICY_2026-04-14.md filed; 3 retrieval rules established; compiled-preferred conditional (Rule 1), raw-source fallback mandatory (Rule 2), no unconditional compiled-first default (Rule 3)` |
| `W72-T3` Benchmark Criteria + Vocabulary Note (DOCUMENTATION class) | `CLOSED DELIVERED 2026-04-14 — CVF_KNOWLEDGE_PREFERENCE_BENCHMARK_CRITERIA_2026-04-14.md filed; promotion gate criteria defined; PROPOSAL_ONLY evidence class defined` |
| `W72-T4` CompiledKnowledgeArtifactContract (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — CompiledKnowledgeArtifactContract in CPF; Compile + Govern lifecycle gates; citationTrail + artifactHash + governedAt canonical fields` |
| `W72-T5` BenchmarkTarget extensions (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — KNOWLEDGE_QUERY / KNOWLEDGE_RANKING / KNOWLEDGE_STRUCTURAL_INDEX / KNOWLEDGE_COMPILED_CONTEXT added to BenchmarkTarget union in performance.benchmark.harness.contract.ts` |
| `W72-T6` W7 Palace Vocabulary Enrichment (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — W7PalaceVocabulary interface; 7 optional palace fields added to W7NormalizedAssetCandidateEnrichment; compile() carries fields through` |
| `W73-T1` W7MemoryRecordContract (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — W7MemoryRecordContract + batch in CPF; palace placement step; 7 optional palace fields; 51 new tests` |
| `W73-T2` KnowledgeMaintenanceContract (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — KnowledgeMaintenanceContract + batch; Step 5 (Maintain) of 6-step lifecycle; 5 check types (lint/contradiction/drift/orphan/staleness); 42 new tests` |
| `W74-T1` KnowledgeRefactorContract (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — KnowledgeRefactorContract + batch; Step 6 (Refactor) of 6-step lifecycle; CLOSES full lifecycle at CPF contract layer; 3 action heuristics (archive/recompile/review); 39 new tests` |
| `W75-T1` KnowledgeContextAssemblyContract (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — KnowledgeContextAssemblyContract + batch; consumer-facing output surface; structural enrichment support; contextWindowEstimate; 39 new tests` |
| `W76-T1` KnowledgeContextAssemblyConsumerPipelineContract (REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — consumer pipeline bridge chaining Ranking → ContextAssembly → ConsumerPackage in one execute() call; structural enrichment flows into packaged context; 30 new tests` |
...
| `W84-T1` Knowledge Live Benchmark Evidence Promotion (VALIDATION_EVIDENCE / GOVERNED_RUNTIME_BENCHMARK class) | `CLOSED DELIVERED 2026-04-14 — 12 live API calls (3 scenarios x 2 runs x 2 paths); model qwen-max (Alibaba DashScope); all 12/12 tests pass; all 8 gates MET; compiled avg 0.667, raw avg 0.556, delta +0.111; decision HYBRID / NO SINGLE DEFAULT CONFIRMED; Gates 1+2 promoted from PROPOSAL_ONLY to LIVE_INFERENCE; benchmark-evidence closure complete, with global canon-front-door closure finalized by W85-T1; Full Lane (GC-018)` |
| `W85-T1` Post-W84 Canon Truth Sync (DOCUMENTATION / CANON_SYNC class) | `CLOSED DELIVERED 2026-04-14 — pre-sync assessment filed; GC-018 authorization filed; whitepaper §4.3 refreshed to post-W84 truth; tracker W84–W85 rows added; AGENT_HANDOFF finalized; GC-026 closure sync filed; front-door canon aligned through W84 evidence; lane W71–W84 globally closure-clean; Full Lane (GC-018)` |
| `W86-T1` PVV Lane Resume — Non-Coder Quality Focus (VALIDATION_EVIDENCE / GOVERNED_RUNTIME_BENCHMARK class) | `CLOSED DELIVERED 2026-04-14 — 40 live runs (20 CFG-A direct Alibaba + 20 CFG-B governed /api/execute); model qwen-max; Gate D MET (0 catastrophic misses); Gate E MET (0 NORMAL over-blocks); Gate A PARTIAL (HIGH_RISK detected but no guided alternative response); NORMAL non-coder parity confirmed (7/7 task classes); HIGH_RISK guidance gap documented (NC-003 ESCALATED 409; NC-006/NC-007 BLOCKED 400); pvv.nc.benchmark.test.ts; CVF_W86_T1_PVV_NC_LIVE_EVIDENCE_PACKET_2026-04-14.md; Full Lane (GC-018)` |
| `W87-T1` HIGH_RISK Guided Response Pattern (PRODUCT / NON_CODER_VALUE / GUIDED_RESPONSE class) | `CLOSED DELIVERED 2026-04-14 — guided.response.registry.ts (3 patterns: NC_003 password storage, NC_006 license violation, NC_007 API key exposure); route.ts enriched (BLOCK + NEEDS_APPROVAL paths inject guidedResponse); 17/17 vitest tests (3 presence + 3 NORMAL undefined + 11 registry structure); Gate A FULL MET (was PARTIAL in W86-T1); Gate D+E maintained; additive only — no policy change, no guard change, no corpus change; CVF_W87_T1_GUIDED_RESPONSE_EVIDENCE_PACKET_2026-04-14.md; Full Lane (GC-018)` |
| `W88-T1` Guided Response UI Realization (PRODUCT / NON_CODER_VALUE / UI_REALIZATION class) | `CLOSED DELIVERED 2026-04-14 — guidedResponse?: string added to ExecutionResponse type; ProcessingScreen.tsx captures data.guidedResponse on BLOCK/NEEDS_APPROVAL and renders bilingual 💡 safe-next-step panel (data-testid="guided-response-panel"); 4 new W88-T1 vitest tests (+1 existing = 5/5 total); non-coder product path now detects AND guides for NC_003/NC_006/NC_007; NORMAL tasks unaffected; no guard/policy change; Fast Lane (GC-021)` |
| `W89-T1` Post-W88 Canon Truth Sync (DOCUMENTATION / CANON_SYNC class) | `CLOSED DELIVERED 2026-04-14 — pre-sync assessment filed; GC-018 authorization filed; whitepaper §4.3 refreshed to post-W88 truth (Last canonical closure, Current active tranche, Current posture W86+W87+W88+W89 entries, Supporting status docs updated); tracker W86–W89 rows added; AGENT_HANDOFF finalized; GC-026 closure sync filed; front-door canon aligned through W88 evidence; lane W71–W88 globally closure-clean; Full Lane (GC-018)` |
| `W90-T1` HIGH_RISK Pattern Expansion (PRODUCT / NON_CODER_VALUE class) | `CLOSED DELIVERED 2026-04-14 — guided.response.registry.ts expanded from 3 to 8 patterns: added NC_001_SQL_INJECTION, NC_002_XSS_OR_UNVALIDATED_INPUT, NC_004_INSECURE_AUTH, NC_005_PII_LOGGING, NC_008_HARDCODED_SECRETS; 32/32 guided.response tests pass; Gate 1 MET; GC-018 authorized` |
| `W91-T1` Template Output Quality Benchmark (VALIDATION_EVIDENCE class) | `CLOSED DELIVERED 2026-04-15 — 9 trusted benchmark wizard templates evaluated on qwen3-max; Gate 2 PASS 9/9 (threshold ≥7/9); scores T1-T9 all ≥7/8; false positive rate 0%; scripts/w91_benchmark.js; GC-018 authorized` |
...
| `W99-T1` OPERATOR Authority Matrix Alignment — Non-Coder Action Verbs (PRODUCT / NON_CODER_VALUE / INFRASTRUCTURE_FIX class) | `CLOSED DELIVERED 2026-04-17 — OFU-1 FIXED; OPERATOR BUILD allowedActions expanded: design, plan, analyze, perform, assess, research, develop, draft (8 verbs added); 9/9 previously W98-blocked scenarios now reach AI execution; 8/9 rubric-passing; Guard Contract 226/226 (+12 new tests); cvf-web 2006/2006 (0 regressions); E2E VALUE PARTIAL improved (4/5 metrics met); sole remaining gap: OFU-2 NC_001 regex; R1 additive; Full Lane (GC-018)` |
| `W98-T1` E2E Success Rate Benchmark — Single Provider Governed Path (PRODUCT / NON_CODER_VALUE / E2E_BENCHMARK class) | `CLOSED DELIVERED 2026-04-17 — E2E VALUE PARTIAL; 23 planned executions (20 base + 3 iterative); 9 scenarios completed full pipeline (8 rubric-scored); 8/8 = 100% usable; 7/7 HIGH_RISK intercepted (6 enforcement + 1 safety filter); zero enforcement false positives on Class A; D1 iterative round proven end-to-end (_previousOutput threading works); authority_gate OPERATOR role blocked 7/23 runs (infrastructure gap, not product gap, OFU-1); NC_001 regex gap (OFU-2); B6 classification gap (OFU-3); R0 benchmark-only, 0 code changes; scripts/w98_e2e_benchmark.js; Full Lane (GC-018)` |
| `W100-T1` NC_001 Regex Expansion — OFU-2 Fix (PRODUCT / NON_CODER_VALUE / BUGFIX class) | `CLOSED DELIVERED 2026-04-17 — OFU-2 CLOSED; NC_001 detector regex expanded with req.query / request.query / url.?input / url.?param alternatives; B1 scenario (req.query concatenation) correctly triggers guided response on BLOCK; +4 tests (36 guided.response pass, 2010 suite pass, 0 regressions). E2E VALUE PROVEN for 1-provider path (All 5 metrics fully MET). R1 additive change; Fast Lane (GC-021)` |
...
- Latest GC-026 tracker sync: `docs/baselines/CVF_GC026_TRACKER_SYNC_W100_T1_CLOSED_2026-04-17.md` (W100-T1 CLOSED DELIVERED; OFU-2 fixed; 5/5 E2E metrics PROVEN; no active tranche)
...
- Active quality assessment: `docs/assessments/CVF_W100_T1_POST_RUN_QUALITY_ASSESSMENT_2026-04-17.md` (W100-T1 post-run assessment; OFU-2 fixed; 5/5 metrics PROVEN)
- Most recent continuation authorization: `docs/baselines/CVF_GC018_W100_T1_OFU2_NC001_REGEX_EXPANSION_AUTHORIZATION_2026-04-17.md` (W100-T1 Fast Lane GC-021 AUTHORIZED; NC_001 Regex Expansion; R1 additive)
