...
| Control-plane `AI Gateway` target-state | three governed surfaces delivered: signal intake (`W1-T4`), gateway consumer (`W1-T4 CP2`), route resolution (`W1-T7`); `GatewayProcessedRequest + RouteDefinition[] → RouteMatchResult → RouteMatchLog` routing path governed; W1-T4 HTTP routing defer closed; multi-tenant auth and NLP-based PII detection remain deferred | `PARTIAL` |
