...
| Execution Observer / feedback loop target-state | one bounded usable observer/feedback slice delivered through `W2-T4`; `ExecutionFeedbackSignal` is the first structured W4 prerequisite surface; re-intake loop and learning-plane storage remain deferred | `PARTIAL` |
