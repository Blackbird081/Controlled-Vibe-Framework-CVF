...
| Control-plane `AI Gateway` target-state | five governed surfaces delivered across W1-T4/W1-T7/W1-T8: signal intake, gateway consumer, route resolution, tenant auth, auth log; `GatewayProcessedRequest → GatewayAuthResult → RouteMatchResult` chain governed; 2 of 3 W1-T4 defers closed (HTTP routing + multi-tenant auth); NLP-based PII detection remains deferred | `PARTIAL` |
