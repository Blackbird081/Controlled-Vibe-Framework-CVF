...
| Learning-plane `Evaluation Engine / feedback loop` | evaluation engine delivered through `W4-T3`; governance signal bridge delivered through `W4-T4`; full chain TruthModel→EvaluationResult→ThresholdAssessment→GovernanceSignal now governed; feedback re-injection loop remains deferred | `PARTIAL` |
