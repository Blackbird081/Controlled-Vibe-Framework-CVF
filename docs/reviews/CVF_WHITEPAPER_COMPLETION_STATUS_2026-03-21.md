...
| `W1-T2` usable intake slice tranche | authorized as the next realization-first control-plane tranche; `CP1` usable-intake contract baseline and `CP2` unified knowledge retrieval contract are implemented; the tranche remains open | `IN PROGRESS / CP1-CP2 IMPLEMENTED` |
...
- the tranche-local execution plan for `W1-T2` is now open and `CP1` + `CP2` are implemented as bounded usable-intake and unified-retrieval contract baselines
