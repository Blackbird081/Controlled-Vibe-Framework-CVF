#!/usr/bin/env python3
"""
Validate packet-specific approval artifact provenance and authority state across supported packet postures.
"""
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer = _extract_field("transition-approval-artifact-issuer", packet_text)
    state = _extract_field("transition-approval-artifact-authority-state", packet_text)
    note = _extract_field("transition approval authority note", packet_text)

    if issuer != expected["issuer"]:
        violations.append({"type": "approval_artifact_issuer_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact issuer does not match posture policy."})
    if state != expected["state"]:
        violations.append({"type": "approval_artifact_authority_state_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact authority state does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_authority_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact authority note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact Authority Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact authority state does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact authority gate")
