#!/usr/bin/env python3
"""
Validate packet-specific approval artifact binding for transition fulfillment against the packet posture.
"""
...
def _build_expectations(packet_type: str, decision: str, entries: list[dict[str, Any]], packet_path: Path) -> dict[str, str]:
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    expected = _build_expectations(packet_type, decision, entries, packet_path)

    families = _extract_field("transition-approval-bound families", packet_text)
    artifact = _extract_field("transition-approval-artifact", packet_text)
    note = _extract_field("transition approval binding note", packet_text)

    if _split(families) != _split(expected["families"]):
        violations.append({"type": "transition_approval_bound_families_mismatch", "path": _rel(packet_path), "message": "Packet transition-approval-bound family set does not match posture policy."})
    if artifact != expected["artifact"]:
        violations.append({"type": "transition_approval_artifact_mismatch", "path": _rel(packet_path), "message": "Packet transition-approval-artifact does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "transition_approval_binding_note_mismatch", "path": _rel(packet_path), "message": "Packet transition approval binding note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact Binding Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact binding does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact binding gate")
