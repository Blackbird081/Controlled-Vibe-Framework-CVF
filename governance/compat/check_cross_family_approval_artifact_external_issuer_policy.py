#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external issuer attestation and trust policy semantics.
"""
...
def _build_expectations(packet_type: str, decision: str) -> dict[str, str]:
    ...
    if approved:
        ...
        return {
            "attestation": "self-attested-governing-issuer",
            "trust_policy": "self-issued-only",
            "note": note,
        }
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    attestation = _extract_field("transition-approval-artifact-external-issuer-attestation", packet_text)
    trust_policy = _extract_field("transition-approval-artifact-third-party-trust-policy", packet_text)
    note = _extract_field("transition approval external issuer policy note", packet_text)

    if attestation != expected["attestation"]:
        violations.append({"type": "approval_artifact_external_issuer_attestation_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external issuer attestation does not match posture policy."})
    if trust_policy != expected["trust_policy"]:
        violations.append({"type": "approval_artifact_third_party_trust_policy_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact third-party trust policy does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_issuer_policy_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external issuer policy note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Issuer Policy Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external issuer policy does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external issuer policy gate")
