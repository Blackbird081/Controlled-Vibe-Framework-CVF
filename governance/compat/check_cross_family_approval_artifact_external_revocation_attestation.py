#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external revocation proof evidence and third-party revocation attestation enforcement semantics.
"""
...
def _build_expectations(packet_type: str, decision: str) -> dict[str, str]:
    ...
    return {
        "revocation_proof_evidence": "unbound",
        "third_party_revocation_attestation_enforcement": "unbound",
        "note": note,
    }
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    revocation_proof_evidence = _extract_field("transition-approval-artifact-external-revocation-proof-evidence", packet_text)
    third_party_attestation = _extract_field("transition-approval-artifact-third-party-revocation-attestation-enforcement", packet_text)
    note = _extract_field("transition approval external revocation attestation note", packet_text)

    if revocation_proof_evidence != expected["revocation_proof_evidence"]:
        violations.append({"type": "approval_artifact_external_revocation_proof_evidence_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation proof evidence does not match posture policy."})
    if third_party_attestation != expected["third_party_revocation_attestation_enforcement"]:
        violations.append({"type": "approval_artifact_third_party_revocation_attestation_enforcement_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact third-party revocation attestation enforcement does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_revocation_attestation_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external revocation attestation note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Attestation Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation attestation does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation attestation gate")
