#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external revocation issuer attestation proof and issuer trust policy semantics.
"""
...
def _build_expectations(packet_type: str, decision: str) -> dict[str, str]:
    ...
    return {
        "issuer_attestation_proof": "unbound",
        "third_party_revocation_issuer_trust_policy": "unbound",
        "note": note,
    }
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_attestation_proof = _extract_field("transition-approval-artifact-external-revocation-issuer-attestation-proof", packet_text)
    third_party_revocation_issuer_trust_policy = _extract_field("transition-approval-artifact-third-party-revocation-issuer-trust-policy", packet_text)
    note = _extract_field("transition approval external revocation issuer attestation note", packet_text)

    if issuer_attestation_proof != expected["issuer_attestation_proof"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_attestation_proof_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation issuer attestation proof does not match posture policy."})
    if third_party_revocation_issuer_trust_policy != expected["third_party_revocation_issuer_trust_policy"]:
        violations.append({"type": "approval_artifact_third_party_revocation_issuer_trust_policy_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact third-party revocation issuer trust policy does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_attestation_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external revocation issuer attestation note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Attestation Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer attestation does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer attestation gate")
