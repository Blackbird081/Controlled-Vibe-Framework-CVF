#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external revocation issuer authority and revocation attestation verification semantics.
"""
...
def _build_expectations(packet_type: str, decision: str) -> dict[str, str]:
    ...
    return {
        "issuer_authority": "unbound",
        "attestation_verification": "unbound",
        "note": note,
    }
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_authority = _extract_field("transition-approval-artifact-external-revocation-issuer-authority", packet_text)
    attestation_verification = _extract_field("transition-approval-artifact-external-revocation-attestation-verification", packet_text)
    note = _extract_field("transition approval external revocation issuer authority note", packet_text)

    if issuer_authority != expected["issuer_authority"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_authority_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation issuer authority does not match posture policy."})
    if attestation_verification != expected["attestation_verification"]:
        violations.append({"type": "approval_artifact_external_revocation_attestation_verification_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation attestation verification does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_authority_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external revocation issuer authority note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Authority Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer authority does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer authority gate")
