...
provenance and invalidation-authority provenance semantics.
"""
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_proof_authority_provenance = _extract_field(
        "transition-approval-artifact-external-revocation-issuer-proof-authority-provenance", packet_text
    )
    issuer_proof_invalidation_authority_provenance = _extract_field(
        "transition-approval-artifact-external-revocation-issuer-proof-invalidation-authority-provenance", packet_text
    )
    note = _extract_field("transition approval external revocation issuer proof authority provenance note", packet_text)

    if issuer_proof_authority_provenance != expected["issuer_proof_authority_provenance"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_authority_provenance_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval-artifact external revocation issuer proof authority provenance does not match posture policy.",
        })
    if issuer_proof_invalidation_authority_provenance != expected["issuer_proof_invalidation_authority_provenance"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_invalidation_authority_provenance_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval-artifact external revocation issuer proof invalidation-authority provenance does not match posture policy.",
        })
    if note != expected["note"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_authority_provenance_note_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval artifact external revocation issuer proof authority provenance note does not match posture policy.",
        })
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Proof Authority Provenance Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer proof authority provenance does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer proof authority provenance gate")
