...
provenance attestation provenance and invalidation-authority provenance attestation
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_value = _extract_field(
        "transition-approval-artifact-external-revocation-issuer-proof-authority-provenance-attestation-provenance", packet_text
    )
    invalidation_value = _extract_field(
        "transition-approval-artifact-external-revocation-issuer-proof-invalidation-authority-provenance-attestation-provenance", packet_text
    )
    note = _extract_field(
        "transition approval external revocation issuer proof authority provenance attestation provenance note", packet_text
    )

    if issuer_value != expected["issuer_proof_authority_provenance_attestation_provenance"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_authority_provenance_attestation_provenance_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval-artifact external revocation issuer proof authority provenance attestation provenance does not match posture policy.",
        })
    if invalidation_value != expected["issuer_proof_invalidation_authority_provenance_attestation_provenance"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_invalidation_authority_provenance_attestation_provenance_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval-artifact external revocation issuer proof invalidation-authority provenance attestation provenance does not match posture policy.",
        })
    if note != expected["note"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_authority_provenance_attestation_provenance_note_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval artifact external revocation issuer proof authority provenance attestation provenance note does not match posture policy.",
        })
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Proof Authority Provenance Attestation Provenance Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer proof authority provenance attestation provenance does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer proof authority provenance attestation provenance gate")
