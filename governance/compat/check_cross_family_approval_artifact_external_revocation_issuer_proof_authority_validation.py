#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external revocation issuer proof timestamp
verification and invalidation-authority validation semantics.
"""
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_proof_timestamp_verification = _extract_field(
        "transition-approval-artifact-external-revocation-issuer-proof-timestamp-verification", packet_text
    )
    issuer_proof_invalidation_authority_validation = _extract_field(
        "transition-approval-artifact-external-revocation-issuer-proof-invalidation-authority-validation", packet_text
    )
    note = _extract_field("transition approval external revocation issuer proof authority validation note", packet_text)

    if issuer_proof_timestamp_verification != expected["issuer_proof_timestamp_verification"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_timestamp_verification_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval-artifact external revocation issuer proof timestamp verification does not match posture policy.",
        })
    if issuer_proof_invalidation_authority_validation != expected["issuer_proof_invalidation_authority_validation"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_invalidation_authority_validation_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval-artifact external revocation issuer proof invalidation-authority validation does not match posture policy.",
        })
    if note != expected["note"]:
        violations.append({
            "type": "approval_artifact_external_revocation_issuer_proof_authority_validation_note_mismatch",
            "path": _rel(packet_path),
            "message": "Packet approval artifact external revocation issuer proof authority validation note does not match posture policy.",
        })
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Proof Authority Validation Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer proof authority validation does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer proof authority validation gate")
