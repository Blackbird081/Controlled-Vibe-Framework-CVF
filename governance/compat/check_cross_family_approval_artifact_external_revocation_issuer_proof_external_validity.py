#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external revocation issuer proof expiry timestamp
and invalidation-source semantics.
"""
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_proof_expiry_timestamp = _extract_field("transition-approval-artifact-external-revocation-issuer-proof-expiry-timestamp", packet_text)
    issuer_proof_invalidation_source = _extract_field("transition-approval-artifact-external-revocation-issuer-proof-invalidation-source", packet_text)
    note = _extract_field("transition approval external revocation issuer proof external validity note", packet_text)

    if issuer_proof_expiry_timestamp != expected["issuer_proof_expiry_timestamp"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_proof_expiry_timestamp_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation issuer proof expiry timestamp does not match posture policy."})
    if issuer_proof_invalidation_source != expected["issuer_proof_invalidation_source"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_proof_invalidation_source_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation issuer proof invalidation source does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_proof_external_validity_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external revocation issuer proof external validity note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Proof External Validity Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer proof external validity does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer proof external validity gate")
