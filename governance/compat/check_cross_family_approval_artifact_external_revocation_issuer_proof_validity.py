#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external revocation issuer proof freshness
and third-party revocation issuer attestation enforcement semantics.
"""
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_proof_freshness = _extract_field("transition-approval-artifact-external-revocation-issuer-proof-freshness", packet_text)
    third_party_revocation_issuer_attestation_enforcement = _extract_field("transition-approval-artifact-third-party-revocation-issuer-attestation-enforcement", packet_text)
    note = _extract_field("transition approval external revocation issuer proof validity note", packet_text)

    if issuer_proof_freshness != expected["issuer_proof_freshness"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_proof_freshness_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation issuer proof freshness does not match posture policy."})
    if third_party_revocation_issuer_attestation_enforcement != expected["third_party_revocation_issuer_attestation_enforcement"]:
        violations.append({"type": "approval_artifact_third_party_revocation_issuer_attestation_enforcement_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact third-party revocation issuer attestation enforcement does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_proof_validity_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external revocation issuer proof validity note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Proof Validity Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer proof validity does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer proof validity gate")
