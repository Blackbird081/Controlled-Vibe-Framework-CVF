#!/usr/bin/env python3
"""
Validate packet-specific approval artifact external revocation issuer proof verification
and third-party revocation issuer trust enforcement semantics.
"""
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    issuer_proof_verification = _extract_field("transition-approval-artifact-external-revocation-issuer-proof-verification", packet_text)
    third_party_revocation_issuer_trust_enforcement = _extract_field("transition-approval-artifact-third-party-revocation-issuer-trust-enforcement", packet_text)
    note = _extract_field("transition approval external revocation issuer verification note", packet_text)

    if issuer_proof_verification != expected["issuer_proof_verification"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_proof_verification_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external revocation issuer proof verification does not match posture policy."})
    if third_party_revocation_issuer_trust_enforcement != expected["third_party_revocation_issuer_trust_enforcement"]:
        violations.append({"type": "approval_artifact_third_party_revocation_issuer_trust_enforcement_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact third-party revocation issuer trust enforcement does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_revocation_issuer_verification_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external revocation issuer verification note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Revocation Issuer Verification Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external revocation issuer verification does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external revocation issuer verification gate")
