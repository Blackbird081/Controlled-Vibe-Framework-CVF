#!/usr/bin/env python3
"""
Validate packet-specific approval artifact expiry timestamp, freshness, and external invalidation authority.
"""
...
def _build_expectations(packet_type: str, decision: str) -> dict[str, str]:
    ...
    return {
        "expiry_timestamp": "unbound",
        "freshness": "unbound",
        "invalidation_authority": "unbound",
        "note": note,
    }
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    expiry_timestamp = _extract_field("transition-approval-artifact-expiry-timestamp", packet_text)
    freshness = _extract_field("transition-approval-artifact-freshness-state", packet_text)
    invalidation_authority = _extract_field("transition-approval-artifact-external-invalidation-authority", packet_text)
    note = _extract_field("transition approval external validity note", packet_text)

    if expiry_timestamp != expected["expiry_timestamp"]:
        violations.append({"type": "approval_artifact_expiry_timestamp_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact expiry timestamp state does not match posture policy."})
    if freshness != expected["freshness"]:
        violations.append({"type": "approval_artifact_freshness_state_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact freshness state does not match posture policy."})
    if invalidation_authority != expected["invalidation_authority"]:
        violations.append({"type": "approval_artifact_external_invalidation_authority_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact external invalidation authority does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_external_validity_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact external validity note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact External Validity Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact external validity state does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact external validity gate")
