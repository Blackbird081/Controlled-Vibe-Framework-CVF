#!/usr/bin/env python3
"""
Validate packet-specific approval artifact fulfillment state across supported packet postures.
"""
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    satisfied = _extract_field("transition-approval-artifact-satisfied families", packet_text)
    pending = _extract_field("transition-approval-artifact-pending families", packet_text)
    note = _extract_field("transition approval fulfillment note", packet_text)

    if _split(satisfied) != _split(expected["satisfied"]):
        violations.append({"type": "approval_artifact_satisfied_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact satisfied family set does not match posture policy."})
    if _split(pending) != _split(expected["pending"]):
        violations.append({"type": "approval_artifact_pending_mismatch", "path": _rel(packet_path), "message": "Packet approval-artifact pending family set does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "approval_artifact_fulfillment_note_mismatch", "path": _rel(packet_path), "message": "Packet approval artifact fulfillment note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Approval Artifact Fulfillment Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet approval artifact fulfillment state does not match posture policy.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family approval artifact fulfillment gate")
