#!/usr/bin/env python3
"""
Validate packet-specific promotion / exception policy against the multi-runtime manifest.
"""
...
def _build_expectations(packet_type: str, entries: list[dict[str, Any]]) -> dict[str, str]:
    ...
    if packet_type == "internal audit evidence snapshot":
        return {
            ...
            "note": "internal audit posture does not promote any runtime family; auditable families remain evidence-visible only and require a separate approval path outside audit",
        }
    if packet_type == "enterprise onboarding snapshot":
        return {
            "promotable": "none",
            "exception_required": ", ".join(all_families) if all_families else "none",
            "note": "enterprise onboarding posture does not promote any runtime family; onboarding-visible families remain informational only and require a separate approval path outside onboarding",
        }
    if packet_type == "production-candidate review snapshot":
        return {
            ...
            "note": "production-candidate review posture does not promote any runtime family; reviewable families require an explicit release approval decision",
        }
    return {
        "promotable": "none",
        "exception_required": ", ".join(promotion_eligible) if promotion_eligible else "none",
        "note": "local-ready posture does not promote any runtime family; reviewable families require a separate publication or release approval decision",
    }
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    promotable = _extract_field("promotable families", packet_text)
    exception_required = _extract_field("exception-required families", packet_text)
    note = _extract_field("promotion exception note", packet_text)

    if _split(promotable) != _split(expected["promotable"]):
        violations.append({"type": "promotable_mismatch", "path": _rel(packet_path), "message": "Packet promotable-family set does not match posture policy."})
    if _split(exception_required) != _split(expected["exception_required"]):
        violations.append({"type": "exception_required_mismatch", "path": _rel(packet_path), "message": "Packet exception-required family set does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "promotion_exception_note_mismatch", "path": _rel(packet_path), "message": "Packet promotion-exception note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Promotion Policy Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet promotion / exception policy does not match manifest-derived posture.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family promotion policy gate")
