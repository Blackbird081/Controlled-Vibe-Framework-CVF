#!/usr/bin/env python3
"""
Validate packet-specific transition fulfillment evidence against the multi-runtime manifest.
"""
...
def _build_expectations(packet_type: str, decision: str, entries: list[dict[str, Any]]) -> dict[str, str]:
    ...
    if approved:
        ...
        return {
            ...
            "artifact": _rel(DEFAULT_MANIFEST),
            "note": note,
        }
...
def build_report(manifest_path: Path, packet_path: Path) -> dict[str, Any]:
    ...
    families = _extract_field("transition-fulfillment families", packet_text)
    artifact = _extract_field("transition-fulfillment artifact", packet_text)
    note = _extract_field("transition fulfillment note", packet_text)

    if _split(families) != _split(expected["families"]):
        violations.append({"type": "transition_fulfillment_families_mismatch", "path": _rel(packet_path), "message": "Packet transition-fulfillment family set does not match posture policy."})
    if artifact != expected["artifact"]:
        violations.append({"type": "transition_fulfillment_artifact_mismatch", "path": _rel(packet_path), "message": "Packet transition-fulfillment artifact does not match posture policy."})
    if note != expected["note"]:
        violations.append({"type": "transition_fulfillment_note_mismatch", "path": _rel(packet_path), "message": "Packet transition fulfillment note does not match posture policy."})
...
def _print_report(report: dict[str, Any]) -> None:
    print("=== CVF Cross-Family Transition Fulfillment Gate ===")
    ...
    if report["violations"]:
        ...
        print("\nFAIL - Packet transition fulfillment evidence does not match manifest-derived posture.")
    else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="CVF cross-family transition fulfillment gate")
