#!/usr/bin/env python3
"""
Export and validate a canonical internal-audit packet posture for the runtime evidence chain.
"""
...
PACKET_PATH = REPO_ROOT / "docs" / "reviews" / "cvf_phase_governance" / "CVF_INTERNAL_AUDIT_PACKET_2026-03-07.md"
...
def main() -> int:
    ...
    _run(
        [
            ...
            str(PACKET_PATH),
            "--owner",
            "CVF internal audit evidence wave",
            "--packet-type",
            "internal audit evidence snapshot",
            "--version-token",
            "v1.7.1",
            "--target-module",
            "Multi-runtime internal audit evidence review",
            "--local-only-constraints",
            "audit-only packet; not a release approval artifact; publication decision deferred",
            "--known-open-risks",
            "implemented-local families remain in the same evidence chain; manual audit review still required before any publication decision",
            "--decision",
            "audit ready",
        ]
    )
