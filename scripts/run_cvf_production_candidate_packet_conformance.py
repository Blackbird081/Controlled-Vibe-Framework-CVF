#!/usr/bin/env python3
"""
Export and validate a second canonical packet posture for production-candidate review.
"""
...
PACKET_PATH = REPO_ROOT / "docs" / "reviews" / "cvf_phase_governance" / "CVF_RELEASE_REVIEW_PACKET_PRODUCTION_CANDIDATE_2026-03-07.md"
...
def main() -> int:
    ...
    _run(
        [
            ...
            str(PACKET_PATH),
            "--owner",
            "CVF production-candidate review wave",
            "--packet-type",
            "production-candidate review snapshot",
            "--version-token",
            "v1.7.1",
            "--target-module",
            "Multi-runtime production-candidate evidence review",
            "--local-only-constraints",
            "review-only packet; not yet approved for publication; release posture remains review-pending",
            "--known-open-risks",
            "implemented-local families remain in the same evidence chain; publication approval is still deferred; rerun packet export after major trace changes",
            "--decision",
            "review pending",
        ]
    )
